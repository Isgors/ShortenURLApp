...
class OrthosRuntimePlugin : Plugin<Project> {
    ...
    override fun apply(project: Project) {
        ...
        androidComponents?.onVariants { variant ->
            ...
            val orthosEnabledForVariant =
                extension.enabled && currentBuildType in extension.enabledBuildTypes

            project.logger.debug("ORTHOS: onVariants CALLED for ${variant.name}")

            // ✅ Single source-of-truth pushed into the app
            variant.buildConfigFields?.put(
                "ORTHOS_ENABLED",
                BuildConfigField(
                    "boolean",
                    orthosEnabledForVariant.toString(),
                    "Generated by Orthos plugin"
                )
            )

            if (!orthosEnabledForVariant) {
